...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="remap-local-filename">
    ...
    <xsl:choose>
      ...
      <xsl:when
          test="$filename = 'gnu general public license v2.0 w/classpath exception - gplv2+ce.html'">
        <xsl:value-of
            select="'gnu general public license v2.0 w_classpath exception - gplv2+ce.html'"/>
      </xsl:when>
      <xsl:otherwise>
