...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="gpl_2_cpe_name" select="'GNU General Public License v2.0 w/Classpath exception'"/>
  <xsl:variable name="gpl_2_cpe_url" select="'http://openjdk.java.net/legal/gplv2+ce.html'"/>

  <xsl:variable name="gpl_2_name" select="'GNU General Public License v2.0 only'"/>
  <xsl:variable name="gpl_2_url" select="'http://www.gnu.org/licenses/old-licenses/gpl-2.0-standalone.html'"/>
  ...
  <xsl:variable name="cddl_10_name" select="'Common Development and Distribution License 1.0'"/>
  <xsl:variable name="cddl_10_url" select="'https://opensource.org/licenses/cddl1.html'"/>

  <xsl:variable name="cddl_11_name" select="'Common Development and Distribution License version 1.1'"/>
  <xsl:variable name="cddl_11_url" select="'https://javaee.github.io/glassfish/LICENSE.html'"/>
  ...
  <xsl:template match="license">
    <xsl:choose>
      ...
      <xsl:when
          test="contains(ancestor::dependency/groupId/text(), 'org.atteo')">
        <xsl:call-template name="license">
      ...
      <xsl:when test="contains(translate(url/text(), $uppercase, $lowercase), 'cddl1.php')">
        <xsl:call-template name="license">
      ...
      <xsl:when test="contains(translate(., $uppercase, $lowercase), 'gpl2 w/ cpe')">
        <xsl:call-template name="license">
          <xsl:with-param name="name" select="$gpl_2_cpe_name"/>
          <xsl:with-param name="url" select="$gpl_2_cpe_url"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="contains(translate(url/text(), $uppercase, $lowercase), 'gpl-2.0')">
        <xsl:call-template name="license">
          <xsl:with-param name="name" select="$gpl_2_name"/>
          <xsl:with-param name="url" select="$gpl_2_url"/>
        </xsl:call-template>
      </xsl:when>
